...
from .webutil import ServerError, optional_arg, required_arg
...
def infer_file_obsid(parent_dirs, name, info):
    ...
    if mode == "hera":
        ...
        if len(obsids) != 1:
            raise ServerError(
                'need to infer obsid of HERA file "%s", but got %d candidate '
                "obsids from similarly-named files",
                name,
                len(obsids),
            )
...
class File(db.Model):
    ...
    type = NotNull(db.String(32))  # noqa: A001
    ...
    def delete_instances(self, mode="standard", restrict_to_store=None):
        ...
        if mode == "standard":
            ...
            raise ServerError(f"unexpected deletion operations mode {mode!r}")
    ...
    def from_dict(cls, source, info):
        ...
        return cls(name, tp, obsid, source, size, md5, datetime.datetime.fromtimestamp(ctime_unix))
    ...
    def make_instance_creation_event(self, instance, store):
        return self.make_generic_event(
            "create_instance", store_name=store.name, parent_dirs=instance.parent_dirs
        )

    def make_instance_deletion_event(self, instance, store):
        return self.make_generic_event(
            "delete_instance", store_name=store.name, parent_dirs=instance.parent_dirs
        )

    def make_copy_launched_event(self, connection_name, remote_store_path):
        return self.make_generic_event(
            "launch_copy", connection_name=connection_name, remote_store_path=remote_store_path
        )

    def make_copy_finished_event(
        self,
        connection_name,
        remote_store_path,
        error_code,
...
class DeletionPolicy:
    ...
    def __init__(self):
        raise AssertionError("instantiation of enum not allowed")
